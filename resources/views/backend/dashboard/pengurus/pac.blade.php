        ...
        <div class="content-wrapper">
          <div class="page-header">
            <h3 class="page-title">
              ...
              </span> Pengurus Korcam 
            </h3>
            <nav aria-label="breadcrumb">
              <ul class="breadcrumb">
                  <a href="{{url('pengurus/add-pac')}}">
                    <button type="button" class="btn btn-gradient-primary btn-rounded btn-fw"><span class="mdi mdi-plus"></span>&nbsp;Pengurus Korcam</button>
                  </a>
              </ul>
            </nav>
          </div>
          ...
          <div class="row">
            <div class="col-12 grid-margin">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Pengurus Korcam</h4>
                  <div class="form-inline" style="padding-top:10px">
                      <label>Pilih Wilayah</label>&nbsp
                      <select id="pilih_wilayah" name="pilih_wilayah" onchange="pilihWilayah();" style="font-size:14px;">
                  ...
                  @if($id_akun != '')
                  <div class="form-inline" style="padding-top:10px">
                      <label>Pilih Korcam</label>&nbsp
                      <select id="pilih_unit" name="pilih_unit" onchange="pilihUnit();" style="font-size:14px;">
                          <option value="all">- semua -</option>
                          @foreach($pilihanPac as $value)
                          @if($id_pac == $value->id_pac)
                          <option value="{{ $value->id_pac }}" selected>{{ $value->nama_pac }}</option>
                          @else
                          <option value="{{ $value->id_pac }}">{{ $value->nama_pac }}</option>
                          @endif
                          @endforeach
                      </select>
                  </div>
