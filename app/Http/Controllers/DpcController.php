...
class DpcController extends Controller
{
    public function index(){
        ...
        if($userID != null){
            $data = Dewan::where('akun.id_level', '3')
                    ->join('akun', 'dewan.id_dewan', 'akun.id_dewan')
                    ->join('level', 'akun.id_level', 'level.id_level')
                    ->get();

            return view('backend.dashboard.dpc.index', compact('data'));
        }
        else{
            return redirect('login');
        }
        return view('backend.dashboard.dpc.index');
    }

    public function add(){
        return view('backend.dashboard.dpc.add');
    }
    ...
    public function edit($id_dewan){
        ...
        return view('backend.dashboard.dpc.edit', ['data' => $data]);
    }
